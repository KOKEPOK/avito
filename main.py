...
def get_weather(city, open_weather_token):
    try:
            ...
            print("Посмотрим прогноз погоды?")
            otvet = input()
            #otvet = boolean(input("Посмотрим прогноз погоды?"))
            #print(f"\n\n\nХотите получить данные на 16 дней?")
            if otvet == "True"|"true"|"da"|"1"|"Da"|"TRUE"|"да"|"ДА"|"Да":

                r = requests.get(fore,
                                 params={'id': idid , 'appid': open_weather_token, 'units': 'metric'}
                                 )
                data = r.json()
